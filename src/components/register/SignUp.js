...
export default function SignUp() {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      // Send email as a query parameter (GET request style in POST method)
      const response = await fetch(`http://localhost:8080/apiv1/otp/send-otp?email=${encodeURIComponent(form.email)}`, {
        method: "POST",
        headers: {
      ...
      setResendTimer(30);  // Start countdown for resend timer
...
const handleVerifyOtp = async () => {
  if (form.otp.length !== 5) {
    setErrors((e) => ({ ...e, otp: "Enter the 5-digit OTP" }));
    return;
  }
  ...
  try {
    ...
    // Sending the OTP verification request using POST method
    const res = await fetch("http://localhost:8080/apiv1/otp/verify-otp", {
      method: "POST", // Changed to POST method
      headers: {
        "Content-Type": "application/json", // Indicating the body content type is JSON
      },
      body: JSON.stringify({ email: form.email, otp: form.otp }), // Sending email and OTP in body
    });

    const text = await res.text();

    if (res.ok && text.includes("verified")) { // Check if the response is ok and contains "verified"
      showToast("Email verified successfully ✅", "success");
      setTimeout(() => setStep(2), 1200); // Move to next step
    } else {
      setErrors((e) => ({ ...e, otp: "Invalid OTP. Please try again." }));
    }
  } catch (error) {
    setErrors((e) => ({ ...e, otp: "Verification failed. Try again." }));
  } finally {
    setOtpVerifying(false); // Stop verifying state
  }
};
          ...
          {step === 1 && (
            <div className="space-y-5">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600">We sent a 5-digit OTP to</p>
              ...
              <button
                ...
                disabled={otpVerifying || form.otp.length !== 5}

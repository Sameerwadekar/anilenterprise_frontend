...
//             <img
//               // src={`http://localhost:8080/product/${product.id}/image`}
//               src="https://www.bing.com/images/search?view=detailV2&ccid=EnXUTsYb&id=CEEBF94C88F3B13FFEDDC956D2EDAA5661E0D027&thid=OIP.EnXUTsYbxf1dviJrd_iclgHaHa&mediaurl=https%3a%2f%2f5.imimg.com%2fdata5%2fTQ%2fCJ%2fCV%2fSELLER-7474209%2f20-liter-bisleri-mineral-water-1000x1000.jpg&exph=500&expw=500&q=bisleri+bottle+20+liter&FORM=IRPRST&ck=3C613931EE5060F2FEF776C8B23FC397&selectedIndex=3&itb=0"
//               alt={product.name}
...
// ─── Dummy product — swap PRODUCT_ID + real API when going live ───────────────
// 🔥 To switch back to real API:
//    1. Import: import { getProductById } from "../utils/proxy/productProxy";
//    2. Import: import { createOrder }    from "../utils/proxy/orderProxy";
//    3. Remove DUMMY_PRODUCT below
//    4. Add useEffect to call fetchProduct() on mount
//    5. Replace handleOrder's payload with your real one

const DUMMY_PRODUCT = {
  id:    "2f102f8c-f1c8-4424-ac34-063d70fdcfaf",
  name:  "20L Aqua Jar",
  brand: "AquaPure",
  liter: 20,
  price: 80,
...
export default function OCO() {
  ...
  const handleOrder = async () => {
    setOrdering(true);
    try {
      // 🔥 Replace this with: await createOrder(orderPayload);
      await new Promise(r => setTimeout(r, 1200)); // simulate API

      showSwal({
        type:        "success",
        title:       "Order Placed! 🎉",
        message:     `Your ${qty} × ${product.name} order for ₹${total} is confirmed. Delivery by 6 PM today.`,
        confirmText: "Great!",
        onConfirm:   () => setQty(1),
      });
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-50 pb-36">
      ...
      <div className="px-4 -mt-10 relative z-10">
        <div className="bg-white rounded-3xl shadow-lg shadow-blue-100 border border-blue-50 p-5 flex items-center gap-4">
          ...
          {/* Illustrated product image — no backend needed */}
          <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center flex-shrink-0 shadow-md">
            <div className="text-center">
              <div className="text-white opacity-90"><DropIcon /></div>
              <p className="text-white text-[10px] font-black -mt-1">{product.liter}L</p>
            </div>
          </div>

          <div className="flex-1 min-w-0">
            <p className="text-xs text-blue-400 font-semibold uppercase tracking-wider mb-0.5">
              {product.brand}
            </p>
            <h2 className="text-base font-extrabold text-gray-800 truncate">{product.name}</h2>
            <div className="flex items-baseline gap-1 mt-1">
            ...
            {product.deposit > 0 && (
              <p className="text-[11px] text-gray-400 mt-0.5">+₹{product.deposit} refundable deposit</p>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-blue-100 shadow-sm p-4">
          ...
          <div className="flex items-center justify-between">
            <button
              onClick={() => setQty((q) => Math.max(1, q - 1))}
              disabled={qty <= 1}
              className="w-11 h-11 flex items-center justify-center rounded-xl bg-blue-50 text-blue-700 hover:bg-blue-100 active:scale-90 transition-all disabled:opacity-40"
            >
              <MinusIcon />
            </button>
            <div className="text-center">
          ...
          <div className="space-y-2">
            ...
            {product.deposit > 0 && (
              <div className="flex justify-between items-center text-sm text-gray-500">
                ...
                <span className="font-semibold">₹{product.deposit * qty}</span>
              </div>
            )}
            <div className="h-px bg-blue-50 my-1"/>
            <div className="flex justify-between items-center">
      ...
      <div className="fixed bottom-16 left-0 w-full px-4 z-40">
        <button
          disabled={ordering}
          onClick={handleOrder}
          className={`w-full py-4 rounded-2xl text-base font-extrabold tracking-wide shadow-lg transition-all ${
            ordering
              ? "bg-gray-300 text-gray-400 cursor-not-allowed"
              : "bg-blue-600 hover:bg-blue-700 active:scale-[0.98] text-white shadow-blue-300"
          }`}
